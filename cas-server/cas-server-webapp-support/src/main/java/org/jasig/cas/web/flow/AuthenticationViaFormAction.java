...
import org.jasig.cas.adaptors.jdbc.CasConst;
...
import org.jasig.cas.util.BadImgVerifyCodeAuthenticationException;
import org.jasig.cas.util.EncryptUtil;
...
public class AuthenticationViaFormAction {
    ...
    public final String validatorCode(final RequestContext context,final Credential credential, final MessageContext messageContext) throws Exception{
        ...
        CasConst.errorMap.put(CasConst.CAS_REDIS_PREFIX + credential.getId(), requestIp);
        Cookie[] cookies = request.getCookies();
		String captcha = "";
		for(Cookie cookie : cookies){
			if(cookie.getName().equals("checkCode")){
				captcha = EncryptUtil.decrypt(cookie.getValue(),EncryptUtil.DEFAULT_KEY);
			}
		}
		
        UsernamePasswordCredential upc = (UsernamePasswordCredential) credential;
        String submitAuthcode = upc.getImgverifycode();
        if (StringUtils.isNotBlank(submitAuthcode)){
            if(StringUtils.equalsIgnoreCase(captcha, submitAuthcode)){
                return "success";
            }else{
                populateErrorsInstance(new BadImgVerifyCodeAuthenticationException(), messageContext);
                return "error";
            }
        }
        return "success";
    ...
    private void populateErrorsInstance(final TicketException e, final MessageContext messageContext){ 
    	try { messageContext.addMessage (new MessageBuilder().error().code(e.getCode()).defaultText(e.getCode()).build()); 
    ...
    protected Event createTicketGrantingTicket(final RequestContext context, final Credential credential,
        ...
        try {
            ...
            final String ticketGrantingTicketId = WebUtils.getTicketGrantingTicketId(context);
            final HttpServletResponse resp = WebUtils.getHttpServletResponse(context);
            Cookie cookie = new Cookie("checkTgt", EncryptUtil.encrypt(ticketGrantingTicketId,EncryptUtil.DEFAULT_TGT_KEY));
